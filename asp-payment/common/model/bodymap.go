...
func (bm BodyMap) SetFormFile(key string, file *goutils.File) BodyMap {
	bm[key] = file
	return bm
}
...
func (bm BodyMap) MarshalXML(e *xml.Encoder, start xml.StartElement) (err error) {
	...
	start.Name = xml.Name{Space: constant.SupplierNull, Local: "xml"}
	if err = e.EncodeToken(start); err != nil {
		return
	}
	for k := range bm {
		if v := bm.GetString(k); v != constant.SupplierNull {
...
func (bm BodyMap) CheckEmptyError(keys ...string) error {
	...
	if len(emptyKeys) > 0 {
		return fmt.Errorf("[%s], %v", "missing required parameter", strings.Join(emptyKeys, ", "))
	}
	return nil
}
