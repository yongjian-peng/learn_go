...
func (c *Client) SetProxyUrl(proxyUrl *url.URL) (client *Client) {
	transport := &http.Transport{
		TLSClientConfig:   &tls.Config{InsecureSkipVerify: true},
		DisableKeepAlives: true,
...
func (c *Client) SendStruct(v interface{}) (client *Client) {
	...
	if err != nil {
		c.err = fmt.Errorf("[%w]: %v, value: %v", errors.New("unmarshal error"), err, v)
		return c
	}
	...
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
		...
		if err = json.Unmarshal(bs, &body); err != nil {
			c.err = fmt.Errorf("[%w]: %v, bytes: %s", errors.New("unmarshal error"), err, string(bs))
			return c
		}
...
func (c *Client) SendBodyMap(bm map[string]interface{}) (client *Client) {
	...
	case TypeJSON:
		...
		if err != nil {
			c.err = fmt.Errorf("[%w]: %v, value: %v", errors.New("marshal error"), err, bm)
			return c
		}
		c.jsonByte = bs
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
		...
		case POST, PUT, DELETE, PATCH:
			...
			case TypeMultipartFormData:
				for k, v := range c.multipartBodyMap {
					...
					} else if ss := goutils.ConvertToString(v); ss != "" {
